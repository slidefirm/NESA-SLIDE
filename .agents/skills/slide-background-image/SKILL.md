...
- `prepare-deck` 必須自動把每個 `<img data-semantic-image="true">` 的相對或 `file:` 本機 asset 以 SHA-256 名稱 byte-preserving stage 到 run 內 `semantic-assets/`，並分別重寫 `neutral.html` 與 `mask-pages/` 可解析的相對 `src`。來源缺檔、格式不支援或 remote `src` 必須 fail；不得靠人工複製目錄讓 mask 或 final HTML 偶然可用。data URL 可保留為 inline asset，但獨立 `<img>`／alt／provenance 不得消失。
...
2. 建立逐頁量測 run：
   ...
   `run.json.semantic_assets` 必須列出 staged asset 的 repository-relative POSIX path、source provenance、SHA-256 與 byte length；確認 `neutral.html` 與每個 `mask-pages/mask-###.html` 的 semantic `<img>` 都載入同一個 staged bytes，才可開始 browser measurement。 
...
4. 將圖片套到隔離 HTML：
   ...
   同一階段也必須把已 stage 的 `semantic_photo` 保持為獨立 `<img data-semantic-image="true">`，但將 final `src` 內嵌為 data URL，並保留 `data-semantic-image-source`、`data-semantic-image-sha256`、`data-semantic-image-staged`、alt、crop 與 focal metadata。這不是把照片併入背景：它使 `file://` browser/PPTX exporter 能保留每張 Photo 頁的 native picture object；missing data URL、source/hash 對不上或被移成 CSS background 都是 blocking fail。
...
通過條件：每頁有正確 Raster 背景 asset；HTML foreground 沒有被取代；Photo 頁的獨立 `semantic_photo` 仍存在、可選取、final `src` 為 data URL 且與 Raster 為不同資產；PPTX package 的 layout XML 含預期背景圖片關係；10 個 raster 只在對應 layout，而每個 Photo 頁另有一個非背景的 native slide `<p:pic>`（alt／cover crop／source hash provenance 對應）；前景文字仍是 native text；沒有 fidelity overlay；`run.json` 的視覺人工審查狀態仍被正確回報。
