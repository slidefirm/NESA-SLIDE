(function () {
  ...
  function materializeUngroupedLayerGeometry(root, members) {
    ...
    const pureTransformTranslation = (transform) => {
      ...
      if (matrix) {
        ...
        const values = matrix[1].split(',').map((item) => parseFloat(item.trim()));
      ...
      if (matrix3d) {
        ...
        const values = matrix3d[1].split(',').map((item) => parseFloat(item.trim()));
    ...
    const snapshots = members.map((member) => {
      ...
      const localWidth = member.offsetWidth || box.width / parentScale;
      const localHeight = member.offsetHeight || box.height / parentScale;
      const translateTokens = String(computedStyle.translate || '').trim().split(/\s+/).filter(Boolean);
      const translateX = translateToken(translateTokens[0], localWidth);
      const translateY = translateToken(translateTokens[1] || '0', localHeight);
      const transformOffset = pureTransformTranslation(computedStyle.transform);

      return {
        member: member,
        // stageBox() measures the visible rectangle, including CSS translate.
        // Compensate pure translations before writing the new absolute left/top;
        // otherwise ungrouping applies the same translate a second time.
        left: (box.left - parentBox.left) / parentScale - borderLeft
          - (translateX + transformOffset.x) / parentScale,
        top: (box.top - parentBox.top) / parentScale - borderTop
          - (translateY + transformOffset.y) / parentScale,
