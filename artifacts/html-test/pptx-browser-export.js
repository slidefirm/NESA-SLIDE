(function (root) {
  ...
  function lineOptions(element) {
    ...
    if (borderSides(element)) return { type: 'none', transparency: 100, width: 0 };
  ...
  function addText(slide, pptx, element, canvas) {
    ...
    const singleLine = element.singleLine === true;
    ...
    slide.addText(text, {
      ...
      margin: 0,
      // SVG metric text is already given a PowerPoint-safe width by the
      // canonical editor.  Do not shrink it: preserve its readable font size
      // and make the XML body explicitly non-wrapping.
  ...
  function addAsymmetricBorders(slide, pptx, element, canvas) {
    ...
    const edges = [
      { name: 'top', x: box.x, y: box.y, w: box.w, h: 0 },
      { name: 'right', x: box.x + box.w, y: box.y, w: 0, h: box.h },
      { name: 'bottom', x: box.x, y: box.y + box.h, w: box.w, h: 0 },
      { name: 'left', x: box.x, y: box.y, w: 0, h: box.h }
    ];
    let added = 0;
