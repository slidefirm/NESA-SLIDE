...
內嵌 SVG 的可見文字也屬於原生文字投影：rotation=0 的 `<text>` 必須以
`getComputedTextLength()` 與 `getBBox()` 量測 viewBox glyph geometry，依
`text-anchor` 投影為 `start`／`middle`／`end` 的單行文字框。該框需保留字級比例
安全寬、`wrap="none"` 與無 shrink-autofit；不得以 glyph 的窄 DOM client rect、縮字或
垂直堆疊取代原始水平標籤。SVG right-angle text rotation 仍是 blocking Gate。
...
對 Master／Custom Layout deck，另執行 `scripts/qa_pptx_master_aware_overflow.py`。它在不修改
PPTX 的情況下沿 `Slide → Custom Layout → Master` 關係解析 native shape、Placeholder、picture、chart
與 table 的 OOXML bbox，並以 5% virtual padding 檢查原始 canvas bounds。系統 `slides_test.py` 仍要執行
並保留原始結果，但它的 python-pptx padding clone 只位移 slide-owned shapes、沒有位移 inherited Layout/
Master geometry；因此 Layout title 出現在它的灰色 padding 時必須記為 disclosed tool limitation，而不是為了
使該工具變綠去改動 layout-authoritative geometry。只有 master-aware checker 與 PowerPoint 原生逐頁 render
都通過，才可將這種 raw false positive 降為工具限制。

如果一個 typed body Placeholder 對應的 seed 已有逐一 materialized 的 native process／scorecard
children，PowerPoint finalizer 必須保留該 Placeholder 但使其文字為空，並在 finalization report 記錄
`content_owner: native-seed-children`。不可把相同 payload 再填入 body Placeholder，否則原生 children
與 fallback text 重複後會造成重疊。
