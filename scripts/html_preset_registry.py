    ...
    *,
    ...
    for row in entries:
        ...
        if status == "published":
            ...
            # Whether the deployed Gallery HTML is present is a deployment question,
            # not a registry-validity one, so it belongs behind check_gallery. Without
            # the gate, importing the renderer requires the deploy artifacts to exist:
            # html_preset_themes loads this registry at import time with
            # check_gallery=False, so a checkout that carries the rules but not the
            # 361 MB of Theme Lab decks could not even import the renderer.
            if check_gallery and gallery_artifact and not (ROOT / gallery_artifact).is_file():
...
def main() -> int:
    ...
    # verify_gallery checks a deployed Theme Lab page per published Preset. A tree
    # that carries the rules but not the 361 MB of deployed decks has no deployment
    # to verify, so the check is not applicable rather than failing. Where the
    # deployment does exist it still runs, and --skip-gallery still forces it off.
    deployment_present = (ROOT / "artifacts" / "deploy" / "theme-html-lab").is_dir()
    check_gallery = not args.skip_gallery and deployment_present
    data = load_preset_registry(
        args.registry.resolve(),
        check_gallery=check_gallery,
        gallery_path=args.gallery.resolve(),
    )
    gallery_state = "verified" if check_gallery else (
        "skipped" if args.skip_gallery else "not-deployed-here"
    )
    print(json.dumps(
        {"id": data["id"], **data["counts"], "gallery": gallery_state, "pass": True},
        ensure_ascii=False,
    ))
    return 0
