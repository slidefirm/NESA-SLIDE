...
const EDIT_GROUP_FIT_VERSION='visible-union-v8-font-ready';
  ...
  function normalizeVerticalStackVisualGaps(area,boxes){
    // A flex line box can end a few pixels before a text glyph's descent.
    // Materialization freezes the glyph box, so carry that real visual bottom
    // forward to every later vertical-stack item (notably a cover title rule)
    // instead of allowing a decorative rule to cut through the last line.
    if(area?.dataset.autoLayout!=='vertical-stack'||boxes.length<2)return boxes;
    const areaRect=area.getBoundingClientRect();
    const scaleY=area.offsetHeight?areaRect.height/area.offsetHeight:1;
    const computed=getComputedStyle(area);
    const rawGap=Number.parseFloat(computed.rowGap||computed.gap||'0');
    const minGap=Number.isFinite(rawGap)?Math.max(0,rawGap/scaleY):0;
    let previous=null;
    return boxes.map((box)=>{
      const normalized={...box};
      if(previous){
        const minTop=previous.top+previous.height+minGap;
        if(normalized.top<minTop)normalized.top=minTop;
      }
      previous=normalized;
      return normalized;
    });
  ...
  function materializeArea(area){
    ...
    const boxes=normalizeVerticalStackVisualGaps(area,children.map((child)=>{
