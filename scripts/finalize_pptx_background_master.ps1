param(
    ...
    # Optional page-level typed selection. When present, the companion
    # finalizer keeps the artifact-tool seed's native chart/table/picture
    # objects but rebuilds Master -> Custom Layout -> Slide with PowerPoint.
...
if (-not [string]::IsNullOrWhiteSpace($SelectionManifest)) {
    $selectionFinalizer = Join-Path $PSScriptRoot 'finalize_pptx_selection_master.ps1'
    if (-not (Test-Path -LiteralPath $selectionFinalizer)) {
        throw "Missing per-page PPTX selection finalizer: $selectionFinalizer"
    }
    & $selectionFinalizer `
        -InputPptx $InputPptx `
