...
def layer(class_name: str, content: Any, *, tag: str = "div", extra: str = "") -> str:
    return f'<{tag} class="{class_name}" data-edit-layer="text" {extra}>{content}</{tag}>'


def full_scene(content: str, composite: str) -> str:
    return (
        f'<div class="el scene" data-edit-kind="visual" '
        f'data-edit-composite="{esc(composite)}" data-visual-balance="content-bounds" '
        f'style="left:0;top:0;width:1728px;height:888px">{content}</div>'
    )


def split_scene(header: str, content: str, composite: str) -> str:
    """Keep heading and body as separate, tightly fitted sibling edit groups."""

    return (
        f'<div class="el scene scene-header" data-edit-kind="visual" '
        f'data-edit-composite="{esc(composite)}-title-group" data-edit-role="title-group" '
        f'data-edit-fit="container" data-edit-fit-children="true" '
        f'style="left:0;top:0;width:1728px;height:888px">{header}</div>'
        f'<div class="el scene scene-content" data-edit-kind="visual" '
        f'data-edit-composite="{esc(composite)}-content-group" data-edit-role="content-group" '
        f'data-edit-fit="container" data-edit-fit-children="true" '
        f'style="left:0;top:0;width:1728px;height:888px">{content}</div>'
    )
...
def scene_header(slide: dict[str, Any]) -> str:
    return (
        f'{layer("scene-title", esc(slide["title"]), tag="h1")}'
        f'{layer("scene-intro", esc(slide["intro"]), tag="p")}'
    )


def item_lines(items: list[str]) -> str:
    return "".join(f'<li>{layer("item-copy", esc(item), tag="span")}</li>' for item in items)


def render_cover(slide: dict[str, Any]) -> str:
    if (
        slide.get("_theme_id") == "restoration-blueprint-ledger"
        and slide.get("layout_id") == "restore-cover-section"
    ):
        visual = (
            '<figure class="el restore-cover-illustration" '
            'data-edit-kind="visual" data-edit-layer="visual" '
            'data-edit-role="hero-illustration" aria-label="老屋立面測繪插畫">'
            '</figure>'
        )
    elif (
        slide.get("_theme_id") == "moonlit-herbarium-atlas"
        and slide.get("layout_id") == "moonlit-cover-greenhouse"
    ):
        visual = (
            '<figure class="el moonlit-greenhouse-illustration" '
            'data-edit-kind="visual" data-edit-layer="visual" '
            'data-edit-role="hero-illustration" aria-label="月夜溫室與授粉者觀測插畫">'
            '</figure>'
        )
    else:
        visual = (
            '<div class="cover-signature" data-edit-layer="visual" '
            'aria-hidden="true"><i></i><i></i><i></i><i></i><i></i></div>'
        )
    body = f'''
      {layer("cover-kicker", esc(slide["kicker"]))}
      {layer("cover-title", rich(slide["title"]), tag="h1")}
      {layer("cover-subtitle", esc(slide["subtitle"]), tag="p")}
      {layer("cover-meta", esc(slide["meta"]))}
      {visual}
    '''
...
def render_thesis(slide: dict[str, Any]) -> str:
    notes = "".join(
        f'<li><b>{index:02d}</b>{layer("note-copy", esc(note), tag="span")}</li>'
        for index, note in enumerate(slide.get("notes", []), 1)
    )
    body = f'''
      <div class="thesis-mark" data-edit-layer="visual" aria-hidden="true">“</div>
      {layer("thesis-quote", rich(slide["quote"]), tag="blockquote")}
      {layer("thesis-attribution", esc(slide["attribution"]))}
...
def render_index(slide: dict[str, Any]) -> str:
    items = "".join(
        f'''<article class="index-item item-{index}">
          {layer("index-label", esc(item["label"]))}
          {layer("index-title", esc(item["title"]), tag="h2")}
          {layer("index-body", esc(item["body"]), tag="p")}
        </article>'''
...
def render_contrast(slide: dict[str, Any]) -> str:
    ...
    for side in ("left", "right"):
        panel = slide[side]
        panels.append(
            f'''<section class="contrast-panel contrast-{side}">
              {layer("contrast-label", "現況" if side == "left" else "目標")}
              {layer("contrast-title", esc(panel["title"]), tag="h2")}
              {layer("contrast-lead", esc(panel["lead"]), tag="p")}
              <ul>{item_lines(panel["items"])}</ul>
            </section>'''
        )
    return split_scene(
        scene_header(slide),
        f'<div class="contrast-grid">{"".join(panels)}</div>',
        slide["layout_id"],
    )
...
def render_columns(slide: dict[str, Any]) -> str:
    ...
    items = "".join(
        f'''<article class="el column-item item-{index}" data-edit-kind="visual"
          data-edit-fit="container" data-edit-composite="{esc(slide["layout_id"])}-column-card-{index}"
          data-edit-structure="module">
          <div class="column-item-bg" data-edit-layer="background" aria-hidden="true"></div>
          {layer("column-tag", esc(item["tag"]))}
          {layer("column-title", esc(item["title"]), tag="h2")}
          {layer("column-body", esc(item["body"]), tag="p")}
        </article>'''
        for index, item in enumerate(slide["items"], 1)
    )
    return split_scene(
        scene_header(slide),
        f'<div class="column-grid count-{item_count}">{items}</div>',
        slide["layout_id"],
    )
...
def render_flow(slide: dict[str, Any]) -> str:
    items = "".join(
        f'''<article class="flow-item item-{index}">
          {layer("flow-label", esc(item["label"]))}
          {layer("flow-title", esc(item["title"]), tag="h2")}
          {layer("flow-body", esc(item["body"]), tag="p")}
        </article>'''
        for index, item in enumerate(slide["items"], 1)
    )
    return split_scene(
        scene_header(slide),
        f'<div class="flow-line" data-edit-layer="visual"></div><div class="flow-list count-{len(slide["items"])}">{items}</div>{layer("scene-footer", esc(slide["footer"]), tag="p")}',
        slide["layout_id"],
    )
...
def render_matrix(slide: dict[str, Any]) -> str:
    items = "".join(
        f'''<article class="matrix-item item-{index}">
          {layer("matrix-q", esc(item["q"]))}
          {layer("matrix-title", esc(item["label"]), tag="h2")}
          {layer("matrix-body", esc(item["body"]), tag="p")}
        </article>'''
        for index, item in enumerate(slide["items"], 1)
    )
    axes = slide["axes"]
    axis_markup = "".join(layer(f"axis-label axis-{index}", esc(value)) for index, value in enumerate(axes, 1))
    return split_scene(
        scene_header(slide),
        f'<div class="matrix-frame" data-edit-layer="visual"><i></i><i></i></div>{axis_markup}<div class="matrix-items">{items}</div>',
        slide["layout_id"],
    )


def render_ledger(slide: dict[str, Any]) -> str:
    headers = "".join(layer("ledger-cell ledger-head", esc(value)) for value in slide["headers"])
    rows = "".join(
        '<div class="ledger-row">' + "".join(layer("ledger-cell", esc(value)) for value in row) + "</div>"
        for row in slide["rows"]
    )
    return split_scene(
        scene_header(slide),
        f'<div class="ledger"><div class="ledger-row ledger-header">{headers}</div>{rows}</div>{layer("scene-footer", esc(slide["note"]), tag="p")}',
        slide["layout_id"],
    )
...
def render_timeline(slide: dict[str, Any]) -> str:
    items = "".join(
        f'''<article class="timeline-item item-{index}">
          {layer("timeline-time", esc(item["time"]))}
          {layer("timeline-title", esc(item["title"]), tag="h2")}
          {layer("timeline-body", esc(item["body"]), tag="p")}
        </article>'''
        for index, item in enumerate(slide["items"], 1)
    )
    return split_scene(
        scene_header(slide),
        f'<div class="timeline-rule" data-edit-layer="visual"></div><div class="timeline-list count-{len(slide["items"])}">{items}</div>',
        slide["layout_id"],
    )
...
def render_map(slide: dict[str, Any]) -> str:
    nodes = "".join(
        f'''<article class="map-node item-{index}">
          {layer("map-label", esc(node["label"]), tag="h2")}
          {layer("map-body", esc(node["body"]), tag="p")}
        </article>'''
    ...
    return split_scene(
        scene_header(slide),
        f'''<article class="map-center">{layer("map-center-title", esc(center["title"]), tag="h2")}{layer("map-center-body", esc(center["body"]), tag="p")}</article>
        <div class="map-nodes count-{len(slide["nodes"])}">{nodes}</div>''',
        slide["layout_id"],
    )
...
def render_metrics(slide: dict[str, Any]) -> str:
    items = "".join(
        f'''<article class="metric-item item-{index}">
          {layer("metric-value", esc(item["value"]))}
          {layer("metric-label", esc(item["label"]), tag="h2")}
          {layer("metric-meaning", esc(item["meaning"]), tag="p")}
        </article>'''
        for index, item in enumerate(slide["items"], 1)
    )
    return split_scene(
        scene_header(slide),
        f'<div class="metric-grid count-{len(slide["items"])}">{items}</div>',
        slide["layout_id"],
    )
...
def render_close(slide: dict[str, Any]) -> str:
    if (
        slide.get("_theme_id") == "restoration-blueprint-ledger"
        and slide.get("layout_id") == "restore-close-care"
    ):
        visual = (
            '<figure class="el restore-toolbox-illustration" '
            'data-edit-kind="visual" data-edit-layer="visual" '
            'data-edit-role="hero-illustration" aria-label="修復工具插畫">'
            '</figure>'
        )
    elif (
        slide.get("_theme_id") == "moonlit-herbarium-atlas"
        and slide.get("layout_id") == "moonlit-close-first-route"
    ):
        visual = (
            '<figure class="el moonlit-close-illustration" '
            'data-edit-kind="visual" data-edit-layer="visual" '
            'data-edit-role="hero-illustration" aria-label="月夜溫室觀測插畫">'
            '</figure>'
        )
    else:
        visual = '<div class="close-signature" data-edit-layer="visual"><i></i><i></i><i></i></div>'
    return full_scene(
        f'''{layer("close-statement", rich(slide["statement"]), tag="h1")}
        {layer("close-body", esc(slide["body"]), tag="p")}
        {layer("close-action", esc(slide["action"]), tag="p")}
        {layer("close-meta", esc(slide["meta"]))}
        {visual}''',
        slide["layout_id"],
    )
...
.scene [data-edit-layer]{position:absolute;margin:0;padding:0}.scene-title{left:0;top:0;width:1500px;font:800 58px/1.14 var(--font-display);letter-spacing:-.035em;color:var(--ink)}.scene-intro{left:0;top:88px;width:1460px;font:500 24px/1.5 var(--font-body);color:var(--muted)}
.scene-footer{left:0;bottom:0;width:100%;min-height:64px;padding:18px 24px!important;border-top:1px solid var(--line);font:600 20px/1.4 var(--font-body);color:var(--ink)}
.folio{position:absolute;left:28px;right:28px;bottom:22px;display:flex;justify-content:space-between;z-index:4;font:600 12px/1 var(--font-utility);letter-spacing:.13em;color:var(--muted);pointer-events:none}
...
def render_slide(slide: dict[str, Any], theme_id: str, index: int, total: int) -> str:
    ...
    folio = f'<div class="folio"><span>{index + 1:02d} / {total:02d}</span><span>{esc(slide["layout_id"])}</span></div>'
    index_tab = (
        f'<div class="el index-tab" data-edit-kind="text" data-edit-fit="container" '
        f'style="left:1650px;top:84px;width:78px;height:58px">{tab}</div>'
        if tab else ""
    )
    ...
    return (
        ...
        f'data-index-tab="{tab}"><div class="content" data-content-area="true">{renderer(render_context)}{index_tab}</div>{folio}</section>'
    )
...
def build(spec: dict[str, Any], output_path: Path) -> dict[str, Any]:
    ...
    slides_html = "".join(render_slide(slide, theme_id, index, len(slides)) for index, slide in enumerate(slides))
    ...
    asset_policy = (
        "embedded-provenance-svg"
        if REDESIGN_THEME_ASSET_PROVENANCE.get(theme_id)
        else "image-free"
    )
    document = f'''<!doctype html>
<!-- CONTENT-FIRST-AUTHORED-DECK theme={esc(theme_id)} slides={len(slides)} -->
<html lang="zh-Hant" data-theme="{esc(theme_id)}" data-theme-id="{esc(theme_id)}" data-theme-kind="authored-open-design" data-style-profile="{esc(theme_id)}" data-design-dialect="{esc(dialect_id)}" data-html-assembly="content-first-authored-v1" data-asset-policy="{asset_policy}" data-theme-label="{esc(spec['display_name'])}">
...
<body>{player}</body></html>'''
    ...
    manifest = {
        ...
        "theme": {
            ...
            "techniques": REDESIGN_THEME_TECHNIQUES[theme_id],
        },
        "html_assembly": {
            "id": "content-first-authored-v1",
            "layers": ["subject", "narrative", "signature", "composition", "theme", "editor"],
            "guardrails": ["single-content-area", "pure-html", "semantic-structure", "minimum-36px-generated-type", "standalone-embedded-svg", "centered-content-group", "split-title-content-groups", "editor-embedded"],
        },
        "asset_provenance": REDESIGN_THEME_ASSET_PROVENANCE.get(theme_id, []),
        "layouts": layout_ids,
