...
def _split_panel(side: str, content: tuple[str, str, list[str]], left: int) -> str:
    ...
    return f'''<div class="el diagram-node split-panel {side}" data-edit-composite="split-{side}" style="left:{left}px;top:150px;width:805px;height:610px">
      <div class="diagram-node-bg" data-edit-layer="background"></div>
...
def _closing_photo_overlay_contact(content: dict[str, Any]) -> str:
    ...
    return f'''<div class="prod-frame closing-frame" data-density="{density}" data-full-bleed-media="true" style="left:-96px;top:-96px;width:1920px;height:1080px">
      ...
      <div class="el diagram-node closing-social-panel" data-edit-composite="closing-social" style="left:940px;top:278px;width:250px;height:520px">
...
def _flow_stages_three(content: dict[str, Any]) -> str:
    ...
    for index in range(len(stages) - 1):
        ...
        stage_arrows.append(
            f'''<svg class="el diagram-connectors sequence-stage-connectors flow-arrow flow-arrow-{index}" data-edit-kind="visual" data-edit-object="connector-arrow" data-edit-connector-index="{index}" style="left:{start - arrow_padding}px;top:{200 + 190 - arrow_padding}px;width:{arrow_width}px;height:{arrow_padding * 2}px" viewBox="0 0 {arrow_width} {arrow_padding * 2}" aria-hidden="true">
    ...
    for index, ((number, title, body, tag), left) in enumerate(zip(stages, positions)):
        cards.append(
            f'''<div class="el diagram-node sequence-stage-card stage-{index + 1}" data-edit-composite="flow-stage-{index + 1}" style="left:{left:.3f}px;top:200px;width:{card_width:.3f}px;height:380px">
              <div class="diagram-node-bg" data-edit-layer="background"></div><span data-edit-layer="metric">{esc(number)}</span>
              <em data-edit-layer="text">{esc(tag)}</em><b data-edit-layer="text">{esc(title)}</b><p data-edit-layer="text">{esc(body)}</p>
            </div>'''
        )
    takeaway = f'''<div class="el diagram-node sequence-takeaway" data-edit-composite="flow-takeaway" style="left:180px;top:640px;width:1368px;height:100px">
    ...
    return _frame(
        content,
        _flow_header(content["title"], content["subtitle"], subtitle_max_width=1550)
...
def _cover_photo_frame(content: dict[str, Any], photo_side: str) -> str:
    ...
    org = _optional_loose_text(
        ...
        style=f"left:{text_left}px;top:820px;width:max-content;height:auto;max-width:650px",
    )
...
def render_production_layout(layout: dict[str, Any], page_content: dict[str, Any] | None = None) -> str | None:
    ...
    if family == "cover":
        ...
        if layout_id == "cover-photo-frame":
            return _cover_photo_frame(content, "left")
