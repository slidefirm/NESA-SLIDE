...
_SEMANTIC_IMAGE_TAG_RE = re.compile(
    r'<img\b(?=[^>]*\bdata-semantic-image\s*=\s*(["\'])true\1)[^>]*>',
    re.IGNORECASE | re.DOTALL,
)
...
_STAGEABLE_IMAGE_SUFFIXES = {".png", ".jpg", ".jpeg", ".webp"}


def _local_semantic_asset_path(raw_src: str, *, source_html: Path) -> Path | None:
    """Resolve a semantic-image file reference without permitting a silent remote fallback."""
    value = str(raw_src or "").strip()
    if not value:
        raise ValueError("Semantic image is missing its src")
    if value.startswith("data:"):
        return None
    if FILE_URI_RE.match(value):
        candidate = _path_from_file_uri(value)
    elif WINDOWS_ABSOLUTE_PATH_RE.match(value) or Path(value).is_absolute():
        candidate = Path(value)
    else:
        parsed = urlparse(value)
        if parsed.scheme or parsed.netloc:
            raise ValueError(
                "Semantic image src must be a staged local file or data URL; "
                f"remote/unsupported source is not portable: {value!r}"
            )
        candidate = source_html.parent / unquote(parsed.path)
    candidate = candidate.resolve()
    if not candidate.is_file():
        raise FileNotFoundError(
            f"Semantic image asset is missing and cannot be staged: {value!r}"
        )
    if candidate.suffix.lower() not in _STAGEABLE_IMAGE_SUFFIXES:
        raise ValueError(
            f"Semantic image asset has unsupported format {candidate.suffix!r}: {value!r}"
        )
    return candidate


def _rewrite_semantic_image_sources(html: str, replacements: dict[str, str]) -> str:
    """Rewrite only independently editable semantic-image src attributes."""
    if not replacements:
        return html

    def replace_tag(match: re.Match[str]) -> str:
        tag = match.group(0)
        src_match = _SRC_ATTRIBUTE_RE.search(tag)
        if src_match is None:
            raise ValueError("Semantic image is missing its src")
        raw_src = src_match.group(3)
        ...
        return tag[: src_match.start()] + rewritten + tag[src_match.end() :]

    return _SEMANTIC_IMAGE_TAG_RE.sub(replace_tag, html)


def _stage_semantic_assets(
    html: str,
    *,
    source_html: Path,
    run_dir: Path,
) -> tuple[dict[str, str], list[dict[str, Any]]]:
    """Byte-preserve local semantic images inside a background run.

    ``neutral.html`` lives at the run root, while individual mask pages live
    one directory deeper. Callers receive root-relative refs and can derive
    the mask-page refs without ever retaining the source machine path.
    """
    stage_dir = run_dir / "semantic-assets"
    replacements: dict[str, str] = {}
    records: list[dict[str, Any]] = []
    seen_sources: set[str] = set()
    for tag_match in _SEMANTIC_IMAGE_TAG_RE.finditer(html):
        ...
        source_asset = _local_semantic_asset_path(src, source_html=source_html)
        if source_asset is None:
            records.append({"source_kind": "data-url", "staged": None})
            continue
        digest = _sha256_file(source_asset)
        staged_name = f"{digest}{source_asset.suffix.lower()}"
        staged_path = stage_dir / staged_name
        stage_dir.mkdir(parents=True, exist_ok=True)
        if staged_path.is_file():
            if _sha256_file(staged_path) != digest:
                raise ValueError(
                    f"Existing staged semantic asset hash mismatch: {staged_path.name}"
                )
        else:
            shutil.copyfile(source_asset, staged_path)
        if _sha256_file(staged_path) != digest:
            raise ValueError(
                f"Staged semantic asset bytes do not match source: {source_asset.name}"
            )
        root_ref = f"semantic-assets/{staged_name}"
        replacements[src] = root_ref
        portable_source, external_source = _portable_or_external_source(source_asset)
        records.append(
            {
                "source": portable_source,
                "source_provenance": external_source,
                "staged": _portable_workspace_path(staged_path, label="staged semantic asset"),
                "relative_ref": root_ref,
                "sha256": digest,
                "byte_length": source_asset.stat().st_size,
            }
        )
    return replacements, records


def _inline_staged_semantic_assets(
    html: str,
    *,
    semantic_assets: object,
    run_dir: Path,
) -> tuple[str, list[dict[str, Any]]]:
    """Embed staged semantic images in final HTML without flattening their DOM nodes.

    Browser PPTX export runs from ``file://`` and cannot fetch a relative image
    through CORS. The final HTML therefore keeps each independent semantic
    ``<img>`` but replaces its staged ``src`` with a data URL, retaining the
    original portable source reference and digest as explicit metadata.
    """
    records_by_ref: dict[str, dict[str, Any]] = {}
    for item in semantic_assets if isinstance(semantic_assets, list) else []:
        if not isinstance(item, dict):
            continue
        reference = item.get("relative_ref")
        staged = item.get("staged")
        digest = item.get("sha256")
        if not isinstance(reference, str) or not isinstance(staged, str) or not isinstance(digest, str):
            continue
        staged_path = _resolve_manifest_path(
            staged,
            run_dir=run_dir,
            label="staged semantic asset",
        )
        if not staged_path.is_file():
            raise FileNotFoundError(f"Staged semantic asset is missing: {reference!r}")
        actual_digest = _sha256_file(staged_path)
        if actual_digest != digest:
            raise ValueError(
                f"Staged semantic asset hash mismatch for {reference!r}: {actual_digest} != {digest}"
            )
        records_by_ref[reference] = {
            "path": staged_path,
            "sha256": digest,
            "staged": staged,
        }
    ...
    def replace_tag(match: re.Match[str]) -> str:
        ...
        if raw_src.startswith("data:"):
            return tag
        record = records_by_ref.get(raw_src)
        if record is None:
            raise ValueError(
                f"Semantic image src was not staged before final embedding: {raw_src!r}"
            )
        rewritten_src = (
            src_match.group(1)
            + src_match.group(2)
            + _raster_data_url(record["path"])
            + src_match.group(2)
        )
        rewritten = tag[: src_match.start()] + rewritten_src + tag[src_match.end() :]
        rewritten = re.sub(
            r'\sdata-semantic-image-(?:source|sha256|staged)="[^"]*"',
            "",
            rewritten,
            flags=re.IGNORECASE,
        )
        insertion = (
            f' data-semantic-image-source="{raw_src}"'
            f' data-semantic-image-sha256="{record["sha256"]}"'
            ' data-semantic-image-staged="true"'
        )
        close_index = rewritten.rfind("/>")
        if close_index < 0:
            close_index = rewritten.rfind(">")
        if close_index < 0:
            raise ValueError("Semantic image tag is malformed")
        embedded.append(
            {
                "source": raw_src,
        ...
        return rewritten[:close_index] + insertion + rewritten[close_index:]

    return _SEMANTIC_IMAGE_TAG_RE.sub(replace_tag, html), embedded
...
def prepare_deck(args: argparse.Namespace) -> None:
    ...
    semantic_replacements, semantic_assets = _stage_semantic_assets(
        original,
        source_html=source,
        run_dir=run_dir,
    )
    staged_html = _rewrite_semantic_image_sources(original, semantic_replacements)
    neutral_html = _inject_before_head_close(staged_html, NEUTRAL_STYLE)
...
"""
        root_page_html = _inject_before_head_close(neutral_html, visibility_style)
        root_page_html = _inject_before_head_close(
            root_page_html,
            SINGLE_SLIDE_MASK_SCRIPT.replace("__TARGET_SLIDE_ID__", spec["id"]),
        )
        if root_mask_html is None:
            root_mask_html = root_page_html
        page_html = _rewrite_semantic_image_sources(
            root_page_html,
            {reference: f"../{reference}" for reference in semantic_replacements.values()},
        )
        page_path = mask_pages_dir / f"mask-{spec['index'] + 1:03d}.html"
        page_path.write_text(page_html, encoding="utf-8")
        mask_page_paths.append(page_path)
    if root_mask_html is None:
        raise ValueError("Prepared deck has no mask page HTML")
    mask_html_path.write_text(root_mask_html, encoding="utf-8")
...
def _layout_safe_zone_contract(record: dict) -> dict:
    ...
    if profile == "half-image":
        if layout == "cover-photo-frame":
            ...
            seam_region = "image-text seam x=40-48%"
...
def apply_deck(args: argparse.Namespace) -> None:
    ...
    final_html, embedded_semantic_assets = _inline_staged_semantic_assets(
        final_html,
        semantic_assets=manifest.get("semantic_assets"),
        run_dir=run_dir,
    )
    final_html = _inject_before_head_close(final_html, background_style)
    final_path.write_text(final_html, encoding="utf-8")
    ...
    manifest.update(
        {
            ...
            "html_asset_policy": "inline-model-and-semantic-bytes-with-adjacent-byte-preserving-source-copy",
            "semantic_asset_embedding": {
                "mode": "data-url-in-final-independent-img",
                "embedded_count": len(embedded_semantic_assets),
                "assets": embedded_semantic_assets,
                "preserves_independent_dom": True,
            },
            "dimension_contract": {
...
def build_parser() -> argparse.ArgumentParser:
    ...
    prepare_parser.add_argument("--run-dir", help="Optional directory under workspace/html-image-background")
    ...
    prepare_deck_parser.add_argument("--run-dir", help="Optional directory under workspace/html-image-background")
