...
function contentText(selectionSlide, row, rowIndex = 0) {
  ...
  if (/before[-_]content|left[-_]content/i.test(String(key))) return content.before_content || content.left_content || content.body || "";
  if (/after[-_]content|right[-_]content/i.test(String(key))) return content.after_content || content.right_content || content.body || "";
