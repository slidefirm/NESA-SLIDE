...
def render_multi_line_chart_svg(
    ...
    *,
    ...
    with matplotlib.rc_context(rc):
        ...
        ax.set_ylabel("指標值（0–100）", fontproperties=font_36, labelpad=24, color="#101112")
...
# Share of the heat-map figure reserved for the column-label band above the
# cells. The HTML renderer derives its row-label geometry from this same
# fraction, so the label column and the plotted cells cannot drift apart and
# read as two separate tables.
HEAT_MAP_HEADER_FRACTION = 0.18
...
def render_heat_map_chart_svg(
    ...
    *,
    ...
    if clean_rows and len(clean_rows) != len(clean_values):
        raise ValueError("heat map row label count must match the row count")
    if clean_rows and not 0 < row_label_width < width:
        raise ValueError("heat map row labels need a label band inside the figure")
    if not clean_rows:
        row_label_width = 0
    if any(
    ...
    with matplotlib.rc_context(rc):
        ...
        ax.set_facecolor("none")
        fig.subplots_adjust(
            left=row_label_width / width,
            right=1,
            bottom=0,
            top=1 - HEAT_MAP_HEADER_FRACTION,
        )
        ...
        for tick in ax.get_xticklabels():
            ...
            tick.set_color("#101112")
        ax.set_yticks([])
        # Row headers are cells of this same table, drawn in the same data
        # coordinates as the values. Positioning them from a second geometry in
        # the HTML layer is what made the label column read as its own card.
        if clean_rows:
            label_band = row_label_width * len(clean_columns) / (width - row_label_width)
            for row_index, label in enumerate(clean_rows):
                patch = matplotlib.patches.Rectangle(
                    (-0.5 - label_band, row_index - 0.5),
                    label_band,
                    1,
                    facecolor="none",
                    edgecolor="#303132",
                ...
                patch.set_gid(f"python-heat-row-header-bg-{row_index + 1}")
                ax.add_patch(patch)
                text = ax.text(
                    -0.5 - label_band / 2,
                    row_index,
                ...
                text.set_gid(f"python-heat-row-header-{row_index + 1}")
        for row_index, row in enumerate(clean_values):
            for column_index, value in enumerate(row):
                text = ax.text(
                    column_index,
