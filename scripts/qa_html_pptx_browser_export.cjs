...
async function main() {
  ...
  try {
    ...
    const framework = await page.evaluate(() => {
      ...
      const saveMenuButtons = [...document.querySelectorAll("#edit-save-menu button")]
        .map((button) => ({
          label: button.getAttribute("aria-label") || "",
          text: (button.textContent || "").trim(),
        }));
      return {
        ...
        pptxButtonVisible: [...visibleButtons, ...saveMenuButtons].some((button) => (
    ...
    const clickPromise = page.evaluate(async () => {
      if (typeof window.EditMode?.exportPptx !== "function") {
        throw new Error("PPTX exporter API is unavailable");
      }
      await window.EditMode.exportPptx();
      return true;
    });
    const [download, result, toolbarClicked] = await Promise.all([
      downloadPromise,
      resultPromise,
      clickPromise,
    ]);
    ...
    const checks = {
      ...
      toolbarClicked: toolbarClicked === true,
