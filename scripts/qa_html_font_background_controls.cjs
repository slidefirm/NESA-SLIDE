...
async function main() {
  ...
  // This QA asserts browser-download HTML export, not File System Access.
  // Force the fallback before page scripts initialize so a non-gesture export
  // cannot stall waiting on a native picker.
  ...
  try {
    ...
    if (!exportDownloadHarness.pass) {
      throw new Error("Font/background QA did not force browser-download export fallback");
    }
