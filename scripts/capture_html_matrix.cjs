    ...
    for (const file of files) {
      ...
      for (let index = 0; index < limit; index += 1) {
        ...
        const qa = await page.evaluate(() => {
          ...
          // Theme Lab guardrail: public HTML slide content is Pattern/geometry-only.
          // Editor toolbar SVG icons live outside the active slide and are not part of
          // this check. Slide backgrounds may use CSS gradients but never image URLs.
          const assetPolicy = document.documentElement.dataset.assetPolicy || 'image-free';
          ...
          for (const el of slide.querySelectorAll('[data-edit-fit="text"]')) {
            ...
            if (excessWidth > widthTolerance || excessHeight > heightTolerance
