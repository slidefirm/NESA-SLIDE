...
def load_story_file(path: Path) -> dict[str, Any]:
    ...
    page_compositions_explicit = "page_compositions" in payload
    ...
    if page_compositions_explicit and not page_compositions:
        raise ValueError(
            "Story file explicitly declares page_compositions but it is empty; "
            "new-deck pages must carry page-keyed content instead of falling back to concept fixtures"
        )
    story["page_compositions"] = page_compositions
    story["_page_compositions_explicit"] = page_compositions_explicit
...
def build_semantic_pages(
    ...
    page_compositions = story.get("page_compositions") or {}
    if story.get("_page_compositions_explicit"):
        missing = [
            entry["page_id"]
            for entry in content_plan
            if entry["page_id"] not in page_compositions
        ]
        if missing:
            raise ValueError(
                "Explicit page_compositions is missing content-plan pages: "
                + ", ".join(missing)
            )
    ...
    for entry in content_plan:
        intent = entry["intent"]
        authored_payload = page_compositions.get(entry["page_id"])
        if authored_payload is not None:
            ...
            if not authored_payload:
                raise ValueError(f"Page composition must not be empty: {entry['page_id']}")
...
def build(
    ...
    # A with-image Layout in an image-planned HTML deck uses the generated
    # slide-level Raster as its image layer unless an independent semantic
    # photo is explicitly attached. Keep that handoff explicit for the
    # downstream image-background experiment and its manifest checks.
    for decision, layout in zip(layout_plan, render_layouts):
    ...
    # Browser capture QA needs the resolved visual-background contract, not
    # merely the assembly recipe.  Presets that explicitly opt out of ambient
    # paint use `none`; without this root marker a pure Pattern deck is
    # incorrectly reported as missing a raster-like ambient background.
    document_background_pattern = str(
        (preset_definition.get("background_pattern") if preset_definition else None)
        or assembly["recipe"]["background_pattern"]
    )
    if preset_definition and (
        str(preset_definition.get("visual_asset_policy") or "")
        == "generated-raster-background-opt-in"
    ):
        document_background_pattern = "none"
    ...
    document = re.sub(
        ...
        (
            ...
            rf'data-background-profile="{assembly["recipe"]["background_pattern"]}" '
            rf'data-background-pattern="{document_background_pattern}" '
    ...
    if preset_definition:
        ...
        preset_background_pattern = document_background_pattern
