(function () {
  ...
  function pointerTargetsAt(clientX, clientY, additiveSelection, directGroupSelection) {
    ...
    // A persisted manual group is an atomic editable object during ordinary
    // pointer interaction. ``elementsFromPoint`` lists a paint-only overlay
    // before the grouped semantic module beneath it, so promote only a manual
    // group that was itself hit at this exact point. Shift/Ctrl and an active
    // group-edit scope intentionally retain their direct-member behavior.
    if (!additiveSelection && !directGroupSelection && !currentGroupEditScope()) {

      const manualGroupTarget = targets.find((target) => {

        const root = editableRoot(target);

        return !!(root && groupPath(root).length > 0);

      });

      if (manualGroupTarget) {

        targets.splice(targets.indexOf(manualGroupTarget), 1);

        targets.unshift(manualGroupTarget);
  ...
  function pptxSvgTextAnchor(node, style) {

    const value = String(

      (style && style.textAnchor) || node.getAttribute('text-anchor') || 'start'
  ...
  function pptxSvgTextRotation(node) {

    const match = String(node.getAttribute('transform') || '').match(
  ...
  function pptxSvgTextPosition(node, position, viewBox, style) {

    const fallback = pptxElementPosition(node, node.closest('.slide').getBoundingClientRect());

    const anchor = pptxSvgTextAnchor(node, style);

    const rotation = pptxSvgTextRotation(node);

    // The DOM client rect of an SVG <text> can be only a glyph-wide sliver.

    // Passing that rectangle to PowerPoint makes otherwise horizontal labels

    // wrap one character per line.  For unrotated text, project SVG glyph

    // metrics in the viewBox coordinate system instead, preserving the

    // source text-anchor at the logical anchor point.

    if (Math.abs(rotation) > 0.01) {

      return { position: fallback, anchor: anchor, measured: false, metrics: null };
    ...
    try {
      ...
      const anchorX = parseFloat(node.getAttribute('x'));
      ...
      const glyphWidth = Math.max(1, bbox.width, measuredWidth);

      const glyphHeight = Math.max(1, bbox.height, fontSize * 1.12);

      // Leave a font-relative safety band for PowerPoint's different glyph

      // metrics.  The band grows the non-anchor side for start/end labels and

      // remains symmetric for centred ticks, so alignment stays faithful.

      const horizontalPadding = Math.max(2, fontSize * 0.12);

      const verticalPadding = Math.max(1.5, fontSize * 0.08);

      const safeWidth = glyphWidth * 1.18 + horizontalPadding * 2;

      const safeHeight = glyphHeight + verticalPadding * 2;

      const logicalLeft = anchor === 'middle'
      ...
      const topLeft = pptxSvgPoint(position, viewBox, logicalLeft, bbox.y - verticalPadding);
      ...
      return {
        ...
        position: {
          ...
          width: pptxRound(safeWidth / viewBox.width * position.width),

          height: pptxRound(safeHeight / viewBox.height * position.height),
        ...
        metrics: {
          ...
          measuredWidth: pptxRound(measuredWidth),

          glyphWidth: pptxRound(glyphWidth),

          safeWidth: pptxRound(safeWidth),

          glyphHeight: pptxRound(glyphHeight),

          safeHeight: pptxRound(safeHeight)
  ...
  function pptxSvgManifestElements(svg, position, slide, slideIndex, elementIndex) {
    ...
    nodes.forEach((node, childIndex) => {
      ...
      if (tag === 'text') {
        ...
        const textProjection = pptxSvgTextPosition(node, position, viewBox, style);
        ...
        output.push({
          ...
          position: textProjection.position,
          ...
          textAlign: textProjection.anchor === 'middle' ? 'center' : textProjection.anchor === 'end' ? 'right' : 'left',
          ...
          // A metrics-projected SVG label is deliberately a single logical

          // line.  The exporter emits wrap="none" and disables autofit so it

          // cannot become a vertical stack in native PowerPoint.

          singleLine: textProjection.measured,

          renderedLineCount: textProjection.measured ? 1 : null,

          fit: textProjection.measured ? 'none' : 'shrink',

          svgTextProjection: textProjection.measured ? 'metrics' : 'fallback',

          svgTextAnchor: textProjection.anchor,

          svgTextMetrics: textProjection.metrics
  ...
  function updateToolbarLayout() {
    ...
    // Keep the top bar at its fixed height and never change slide/canvas
    // geometry. When editor chrome gets narrow, keep labelled primary actions
    // (insert/save/edit) while secondary actions collapse to their existing
    // accessible icon buttons rather than overflowing or reflowing slides.
    const compactToolbar = editMode && barInner.clientWidth > 0 && barInner.clientWidth <= 760;
    const compactIconButtons = new Set([undoBtn, redoBtn, imageUploadBtn, appearanceBtn]);

    if (editModeLabel) editModeLabel.style.display = editMode && !compactToolbar ? '' : 'none';
    ...
    labeledToolbarBtns.forEach(({ button, label }) => {
      ...
      // Primary action names stay visible. Secondary controls retain their
      // title and aria-label when collapsed to icon-only compact form.
      label.style.display = editMode || button === editBtn ? '' : 'none';
      if (compactToolbar && compactIconButtons.has(button)) label.style.display = 'none';

      button.style.padding = compactToolbar && compactIconButtons.has(button) ? '0 7px' : '0 10px';
